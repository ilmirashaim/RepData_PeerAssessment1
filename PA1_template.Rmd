...
The mean of the steps is `r format(meanSteps)`.  
The median is equal to `r medianSteps`.  
...
```
The interval with maximum steps number is `r maxStepsInterval`.  
...
```
There are `r uncompleteCasesTotal` missing values.  
...
The new mean is `r format(meanAfterFilling)` which is slightly smaller comparing to the old mean `r format(meanSteps)`.  
The new median is `r medianAfterFilling`. The old median was also greater: `r medianSteps`.
